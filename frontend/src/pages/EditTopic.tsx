...
export function EditTopic() {
  ...
  const fetchTopic = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/courses/${courseId}/discussions/topics/${topicId}`, {
  ...
  useEffect(() => {
    if (topicId && courseId) fetchTopic();
  }, [topicId, courseId]);
            ...
            <div style={{ borderTop: '2px solid #f0f0f0', paddingTop: '25px' }}>
              <div style={{ marginBottom: '15px' }}>
                ...
                <div style={{ color: '#333', fontSize: '16px', fontWeight: '500' }}>
                  {topic.created_at ? new Date(topic.created_at).toLocaleDateString('zh-CN') : '-'}
                </div>
              </div>
              <div>
                <div style={{ color: '#999', fontSize: '14px', marginBottom: '5px' }}>
